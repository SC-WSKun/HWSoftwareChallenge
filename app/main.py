...
def Robot_Distance(index, i, j):
    ...
    """
    path1, distance1 = bfs(
        C_to_N(robot[index].x, robot[index].y), C_to_N(i, j)
    )  # 机器人到货物的路径和距离
    if path1 == None:  # 找不到路径
        return [], float("inf")
    try:
...
def Init():
    ...
    for i in range(berth_num):  # 10个泊位
        ...
        berth_benefit[tmp_berth[i].benefit]=i
    benefit_key = sorted(berth_benefit.keys(),reverse = True)
    for i in range(limit_berth):
        b_id = berth_benefit[benefit_key[i]]
        berth[b_id] = tmp_berth[b_id]
...
def Robot_donot_have_goods(index):
    for i in range(
        ...
        robot[index].pop()
        robot[index].x, robot[index].y = N_to_C(robot[index].path[0])
        if (robot[index].x, robot[index].y) == (robot[index].Goods.x, robot[index].Goods.y):    # 货物在脚下
            print("get", index)
    ...
    while direction != []:  # 检验另外三个方向还能不能走，能走就走
        ...
        for i in range(robot_num):  # 检验如果那样走的话会不会和别的机器人碰撞
            ...
            robot[index].insert(a)
            berth_index = robot[index].berth_id
            if (len(robot[index].path) - 1) <= (robot[index].Goods.life - 1):  # 还能继续拿这个货物
                berth[berth_index].robot_undo(berth[berth_index].robot_arrive_time + 2)
            else:  # 拿不了这个货物了，机器人变成无目标货物状态
        ...
        if robot[index].path == []:  # 无目标货物
            ...
            for i in range(len(goods)):
                ...
                if (
                    ...
                    continue
                if (
                        goods[i].value / (len(berth[berth_index].all_path[C_to_N(goods[i].x, goods[i].y)]) - 1)
                        > max_cost_performance
                ):  # 拿取这个货物G性价比更高，把相关信息记录下来
                    max_cost_performance = goods[i].value / (
                            len(berth[berth_index].all_path[C_to_N(goods[i].x, goods[i].y)]) - 1
                    )
                    cargo = goods[i]
            ...
            path2.pop()
            robot[index].path = path1 + path2 + path3
            final_distance = len(robot[index].path) - 1

            robot[index].Goods = cargo
            ...
            # berth[index].future_goods[index] = [id + final_distance - 1, goods[good_index].value]
            berth[berth_index].robot_pull(
                id + final_distance - 1, cargo.value
            )


            if (robot[index].x, robot[index].y) == (cargo.x, cargo.y):  # 货物就在机器人脚下
                print("get", index)
                cargo.is_carried = True
                robot[index].goods = 1
                Robot_have_goods(index)
            else:  # 货物不在机器人脚下
                Robot_donot_have_goods(index)
