...
Index = 0    # 第一帧中运行的bfs的次数
BFS_TIME = []    # 记录每次bfs运行的时间/总时间/平均时间

BFS_NUMBER = 1    # 当前帧BFS运行的次数
BFS_LIMINATION = 1    # 每一帧运行BFS次数的上限
...
def get_adjacent_table():
    ...
    :return:
    '''
    global ch, adjacency_table,berth, boat_capacity, boat
...
def Robot_Distance(index, i, j):
    ...
    '''
    path1, distance1 = bfs(C_to_N(robot[index].x, robot[index].y), C_to_N(i, j))  # 机器人到货物的路径和距离
    if path1 == None:  # 找不到路径
        return [], float('inf')
    try:
        berth_index = robot[index].berth_id
        path2 = berth[berth_index].all_path[C_to_N(i, j)][:]
        path2.pop()
        path2 = path2[::-1]
    except:  # 港口与物品之间不连通
        return [], float('inf')
...
def life_minus_one():
    '''
    货物生命减1，再把生命为0的货物删除掉，然后清空new_goods,最后把BFS_NUMBER置为1
    :return:
    '''
    global goods, new_goods, BFS_NUMBER
            ...
            if robot[i].goods == 1:  # 当前机器人有货物
                for j in goods:    # 找到需要舍弃的货物
                    if j.x == robot[i].targetX and j.y == robot[i].targetY:
                        G = j
                        break
                new_gain = new_cargo[2] / new_distance - G.value/(len(robot[i].path)-1)    # 计算如果抛弃当前货物的话有多少增益
...
def Robot_have_goods(index):
    ...
    :return:
    '''
    for i in range(robot_num):  # robot[index].path[1]记录的是当前这个机器人要走的下一个地方，这个for循环用于判断能不能按照既定路线移动
        ...
        if (robot[index].x, robot[index].y) == (berth[berth_index].x, berth[berth_index].y):
            print("pull", index)
            G = None
        ...
        return
    # 不能移动
    direction = [robot[index].path[0] - n, robot[index].path[0] - 1, robot[index].path[0] + 1, robot[index].path[0] + n]
    ...
    while direction != []:  # 检验另外三个方向还能不能走，能走就走
        ...
        if ch[coordinate_x][coordinate_y] == '*' or ch[coordinate_x][coordinate_y] == '#':  # 海洋和障碍不能走
            continue
...
def Robot_donot_have_goods(index):
    for i in range(robot_num):  # robot[index].path[1]记录的是当前这个机器人要走的下一个地方
        if N_to_C(robot[index].path[1]) == (robot[i].x, robot[i].y):
            break
    else:  # 能移动
        print("move", index, movement_direction[robot[index].path[1] - robot[index].path[0]])
...
def Robot_control(index):
    ...
    if robot[index].status == 0:  # 机器人处于恢复状态，动不了
        # 当前的算法不完善
        if robot[index].path != [] and robot[index].goods == 0:  # 机器人有目标货物但是手上没货物
        ...
        if robot[index].path == []:  # 无目标货物
            ...
            for i, G in enumerate(goods):
                if G.reserve == True:  # 这个货物被选中了
                    continue
                if C_to_N(G.x, G.y) not in berth[berth_index].all_path:  # 无法到达这个货物，不连通，直接看下一个货物
                    continue
                if (len(berth[berth_index].all_path[C_to_N(robot[index].x, robot[index].y)]) + len(
                        berth[berth_index].all_path[C_to_N(G.x, G.y)]) - 2) > G.life:  # 这个货物生命值过低，拿不到
                    continue
                if G.value / (len(berth[berth_index].all_path[C_to_N(G.x, G.y)]) - 1) > target_good[
                    0]:  # 拿取这个货物G性价比更高，把相关信息记录下来
                    target_good[0] = G.value / (len(berth[berth_index].all_path[C_to_N(G.x, G.y)]) - 1)
                    target_good[1] = G.x
                    target_good[2] = G.y
                    good_index = i
            ...
            BFS_NUMBER += 1
            # 机器人
            robot[index].path, final_distance = Robot_Distance(index, goods[good_index].x, goods[good_index].y)
            ...
            berth[berth_index].robot_pull(id+final_distance-1,goods[good_index].value)
